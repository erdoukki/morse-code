...
Fortunately, the Raspberry Pi has all the above circuitry built in and we can select either a pull up or a pull down circuit in our code for each GPIO pin. This sets up some internal circuitry that is too small for us to see. So you can get away with just using two jumper wires here, although you're welcome to wire it up the way shown above if you wish. Let's use pin #4 as an example:
...
Either copy and paste or enter the code below. Pay attention to the `GPIO.setup` command. This line is doing two things: it is setting pin 4 as an input, **and** setting the internal pull up resistor on it. If you want to use the pull down resistor, you'll need to use `GPIO.PUD_DOWN` instead.

There is then a `while` loop, which continually reads the state of pin 4 and prints HIGH or LOW to the screen every second.
